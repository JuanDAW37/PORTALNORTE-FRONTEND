...
@Component({
  selector: 'app-gestors-modif',
  templateUrl: './gestors-modif.component.html',
  styleUrls: ['../../../app.component.css']
})
export class GestorsModifComponent {
  ...
  constructor(
    ...
    if (this.gestorsService.estado) {
      ...
      this.uploadForm.controls['contrato'].disable();
      this.uploadForm.controls['sueldo'].disable();
      ...
      this.uploadForm.controls['contrato'].removeValidators([Validators.required, Validators.minLength(1)]);
      this.uploadForm.controls['sueldo'].removeValidators([Validators.required, Validators.minLength(1)]);
      ...
      this.titulo="Consulta de Gestor";
    }else{
      this.titulo="Modificación de Gestor";
      this.uploadForm.controls['password'].disable();
    }
    };

    ngOnInit(): void {
      this.gestorsService.leer(this.id).subscribe((data)=>{
        ...
        this.uploadForm.controls['contrato'].setValue(data.contrato);
        this.uploadForm.controls['sueldo'].setValue(data.sueldo);
  ...
  */
  guardar():void {
    try{
      ...
      gestor.editpassword=this.edit_password;
      gestor.id=this.id;
      ...
      this.gestorsService.update(gestor).subscribe((data) => {
          ...
          for (let i = 0; i < this.email.length ; i++) {
            ...
            this.email_service.leer(email.id).subscribe((data)=>{
              if(!data.status){
                ...
                nuevo_email.gestor_id=email.gestor_id;
          ...
          for (let i = 0; i < this.telef.length ; i++) {
            ...
            this.telef_service.leer(tel.id).subscribe((data)=>{
              if (!data.status) {
                ...
                nuevo_tel.gestor_id=tel.gestor_id;
