...
@Component({
  selector: 'app-clientes-modif',
  templateUrl: './clientes-modif.component.html',
  styleUrls: ['./clientes-modif.component.css']
})
export class ClientesModifComponent {
  ...
  uploadForm=new FormGroup({
    ...
    bonificacion: new FormControl(0),
  ...
  constructor(
    ...
    private imagen: ImagenService){
      ...
      this.telef=[];
      this.email=[];
      if (this.clientesService.estado) {
        this.uploadForm.controls['nombre'].disable();
        this.uploadForm.controls['apellido1'].disable();
        this.uploadForm.controls['apellido2'].disable();
        this.uploadForm.controls['nif'].disable();
        this.uploadForm.controls['foto'].disable();
        this.uploadForm.controls['user'].disable();
        this.uploadForm.controls['password'].disable();
        this.uploadForm.controls['bonificacion'].disable();
        this.uploadForm.controls['date'].disable();
        this.uploadForm.controls['calle'].disable();
        this.uploadForm.controls['km'].disable();
        this.uploadForm.controls['numero'].disable();
        this.uploadForm.controls['bloque'].disable();
        this.uploadForm.controls['piso'].disable();
        this.uploadForm.controls['letra'].disable();
        this.uploadForm.controls['cp'].disable();
        this.uploadForm.controls['ciudad'].disable();
        this.uploadForm.controls['provincia'].disable();
        this.uploadForm.controls['pais'].disable();
        this.uploadForm.controls['nTelef'].disable();
        this.uploadForm.controls['dEmail'].disable();
        this.uploadForm.controls['tblTelef'].disable();
        this.uploadForm.controls['tblEmail'].disable();
        this.uploadForm.controls['nombre'].removeValidators([Validators.required, Validators.minLength(1)]);
        this.uploadForm.controls['apellido1'].removeValidators([Validators.required, Validators.minLength(1)]);
        this.uploadForm.controls['nif'].removeValidators([Validators.required, Validators.minLength(4),Validators.maxLength(15)]);
        this.uploadForm.controls['user'].removeValidators([Validators.required, Validators.minLength(1)]);
        this.uploadForm.controls['password'].removeValidators([Validators.required, Validators.minLength(1)]);
        this.uploadForm.controls['calle'].removeValidators([Validators.required, Validators.minLength(2)]);
        this.uploadForm.controls['cp'].removeValidators([Validators.required, Validators.minLength(5)]);
        this.uploadForm.controls['ciudad'].removeValidators([Validators.required, Validators.minLength(4)]);
        this.uploadForm.controls['provincia'].removeValidators([Validators.required, Validators.minLength(4)]);
        this.uploadForm.controls['pais'].removeValidators([Validators.required, Validators.minLength(4)]);
  ...
  ngOnInit(): void {
    console.log(this.id)
    this.clientesService.leer(this.id).subscribe((data)=>{
      this.uploadForm.controls['nombre'].setValue(data.nombre);
      this.uploadForm.controls['apellido1'].setValue(data.apellido1);
      if(data.apellido2==null){
        this.uploadForm.controls['apellido2'].setValue('')
      }else{
        this.uploadForm.controls['apellido2'].setValue(data.apellido2);
      }
      this.uploadForm.controls['nif'].setValue(data.nif);
      this.uploadForm.controls['foto'].setValue(data.foto);
      this.uploadForm.controls['user'].setValue(data.user);
      this.uploadForm.controls['password'].setValue(data.password);
      this.uploadForm.controls['date'].setValue(data.baja);
      this.uploadForm.controls['bonificacion'].setValue(data.bonificacion);
      this.uploadForm.controls['calle'].setValue(data.calle);
      this.uploadForm.controls['km'].setValue(data.km);
      this.uploadForm.controls['numero'].setValue(data.numero);
      this.uploadForm.controls['bloque'].setValue(data.bloque);
      this.uploadForm.controls['piso'].setValue(data.piso);
      this.uploadForm.controls['letra'].setValue(data.letra);
      this.uploadForm.controls['cp'].setValue(String(data.cp));
      this.uploadForm.controls['ciudad'].setValue(data.ciudad);
      this.uploadForm.controls['provincia'].setValue(data.provincia);
      this.uploadForm.controls['pais'].setValue(data.pais);
      this.direccion_id=data.direccione_id;
      if(data.telefono.length>0){
        for (var i = 0; i < data.telefono.length ; i++) {
          let telef=new Telefonos();
          telef.numero=data.telefono[i].numero;
          telef.id=data.telefono[i].id;
      ...
      if(data.email.length>0){
        for (var i = 0; i < data.email.length ; i++) {
          ...
          email.email=data.email[i].email;
          email.id=data.email[i].id;
  ...
  compruebaNIF(event:any):void{
    if((this.uploadForm.controls['nif'].value!=undefined)||(this.uploadForm.controls['nif'].value!=null)){
      ...
      let user="";
      this.clientesService.nifUser(nif, user).subscribe((data)=>{
        if(data.status){
  ...
  change_password():void{
    ...
    this.uploadForm.controls['password'].enable();
  }
    ...
    if(noexiste){
      this.telef_service.filtrar(telef).subscribe((data)=>{
        if (data.status){
          ...
          this.uploadForm.controls['nTelef'].setValue("");
        }else{
    ...
    if(noexiste){
      this.email_service.filtrar(email).subscribe((data)=>{
        if (data.status){
          ...
          this.uploadForm.controls['dEmail'].setValue("");
        }else{
          let ema=new Emails();
          ema.email=email;
          //Si estamos en modo edición de email, eliminamos el anterior email y metemos el nuevo
 ...
 */
  subir():any{
    try{
      ...
      this.imagen.cliente(formData).subscribe((data)=>{
  ...
  */
    guardar():void{
      try{
        ...
        cliente.nombre = String(this.uploadForm.controls['nombre'].value);
        cliente.nif = String(this.uploadForm.controls['nif'].value);
        cliente.apellido1= String(this.uploadForm.controls['apellido1'].value);
        cliente.apellido2= String(this.uploadForm.controls['apellido2'].value);
        cliente.baja=String(this.uploadForm.controls['date'].value);
        cliente.bonificacion=parseFloat(String(this.uploadForm.controls['bonificacion'].value));
        cliente.user=String(this.uploadForm.controls['user'].value);
        cliente.password=String(this.uploadForm.controls['password'].value);
        cliente.editpassword=this.edit_password;
        cliente.id=this.id;
        if(this.archivo!=undefined){
          cliente.foto=this.archivo.name;
        }else{
          cliente.foto="";
        }
        cliente.direccione_id = this.direccion_id;
        this.clientesService.update(cliente).subscribe((data) => {
            ...
            for (let i = 0; i < this.email.length ; i++) {
              ...
              this.email_service.leer(email.id).subscribe((data)=>{
                if(!data.status){
                  ...
                  nuevo_email.cliente_id=email.cliente_id;
            ...
            for (let i = 0; i < this.telef.length ; i++) {
              ...
              this.telef_service.leer(tel.id).subscribe((data)=>{
                if (!data.status) {
                  ...
                  nuevo_tel.cliente_id=tel.cliente_id;
  ...
  buscarDireccion():void{
    ...
    this.direccion.filtrar(calle, km, numero, bloque, piso, letra).subscribe((respuesta) => {
        if (!respuesta.status) {
          ...
          this.uploadForm.controls['cp'].setValue(String(respuesta.cp));
