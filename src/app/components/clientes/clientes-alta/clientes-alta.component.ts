...
@Component({
  ...
  styleUrls: ['./clientes-alta.component.css']
})
  ...
  */
  guardar():void{
    try{
      ...
      cliente.baja=this.uploadForm.controls['date']?.value;
      cliente.bonificacion=parseFloat(this.uploadForm.controls['bonificacion'].value);
      ...
      this.clientesService.insert(cliente).subscribe((data) => {
        if(cliente.foto!=""){
          this.subir();
        }
        let cliente_id=data.id;
        if (this.email.length>0){
          for (let i = 0; i < this.email.length ; i++) {
            let em=new Emails();
            em.email=this.email[i].email;
            em.cliente_id=cliente_id;
